...
app.get("/api/my-salary/:lineUserId", async (req, res) => {
  try {
    ...
    const transportAllowance =
      Number(emp.transport_allowance || emp.attendance_bonus || 3000);
    ...
    function toTaipeiDate(dateValue){
    ...
    function calcBreakHours(startTime,endTime){
      const start = toTaipeiDate(startTime);
      const end = toTaipeiDate(endTime);
      ...
      const overlapStart =
        start > lunchStart ? start : lunchStart;

      const overlapEnd =
        end < lunchEnd ? end : lunchEnd;
    ...
    for (const day of Object.values(dayGroups)) {
      if (day.start && day.end) {
        ...
        const taipeiStart = toTaipeiDate(startTime);
        ...
        const breakDeduct =
          calcBreakHours(startTime,endTime);
